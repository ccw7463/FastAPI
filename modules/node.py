    ...
    def Node__extract_info(self):
        '''
            Des:
                사용자 요청에서 정보 추출
        '''
        self.USE_INFO["USE_FACIL"] = check_yes(Chain__check_facil.invoke(self.STATE).content)
        self.USE_INFO["USE_IO"] = check_yes(Chain__check_io.invoke(self.STATE).content)
        self.USE_INFO["USE_COUNTRY"] = check_yes(Chain__check_country.invoke(self.STATE).content)
        ...
        for key in self.USE_INFO.keys():
            ...
            if key == "USE_TIME" and self.USE_INFO.get(key) == "YES":
                self.EXTRACT_INFO_OBJ.set_time_info(Chain__extract_time.invoke(self.STATE).content)
            ...
            if key == "USE_IO" and self.USE_INFO.get(key) == "YES":
                self.EXTRACT_INFO_OBJ.set_IO_info(Chain__extract_io.invoke(self.STATE).content)
            if key == "USE_COUNTRY" and self.USE_INFO.get(key) == "YES":
                self.EXTRACT_INFO_OBJ.set_COUNTRY_info(Chain__extract_country.invoke(self.STATE).content)                
