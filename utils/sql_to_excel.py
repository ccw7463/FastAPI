...
class ExcelTemplateApplier:
    ...
    def set_time_and_path(self,NOW):
        ...
        self.SAVE_PATH = f'data/SQL2Excel/{formatted_date}_통계추출결과'
    ...
    def extract_common_parts(self):
        ...
        self.INFO_DEFAULT = [f" * {INFO_MESSAGE}"," * 통합포트미스의 자료와 상이할 수 있습니다."]
    ...
    def yes_template_common_parts(self,PATH):
        ...
        if column_row_index is not None:
            DF_FINAL = cleaned_df.iloc[column_row_index:].reset_index(drop=True)
    ...
    def no_template(self):
        ...
        for num,val in enumerate(self.LLM_COLS):    
            ...
            WS[f"{alpha}7"] = val
            WS[f"{alpha}7"].fill = PatternFill(start_color="cae8ff", end_color="cae8ff", fill_type="solid")
            WS[f"{alpha}7"].font = Font(bold=True)
            WS[f"{alpha}7"].alignment = Alignment(horizontal="center", vertical="center")
            WS.column_dimensions[alpha].width = 3+select_width(val,str(self.ONE_SAMPLE[num]))
    ...
    def yes_template_with_template_cols(self):
        ...
        # 컬럼 알파벳 목록 추출
        COLS_ALPHA_ALL = get_alphabets_between(TABLE_COL_START_ALPHA, TABLE_COL_END_ALPHA)
        ...
        for alpha in COLS_ALPHA_ALL:
            if alpha in list(self.WS.column_dimensions.keys()):
                ...
                self.WS.column_dimensions[alpha]=self.WS.column_dimensions[chr(ord(alpha)-1)]
    ...
    def yes_template_with_llm_cols(self):
        ...
        for col_idx,col_val in zip(range(NEW_TABLE_COL_START_IDX,NEW_TABLE_COL_END_IDX+1),self.LLM_COLS):   
            ...
            NEW_WS[AlphaNum].border = NEW_CELL_STYLES[AlphaNum]["border"]
            NEW_WS[AlphaNum].alignment = NEW_CELL_STYLES[AlphaNum]["alignment"]
            NEW_WS.column_dimensions[alpha].width = 3+NEW_CELL_STYLES[AlphaNum]["width"]
...
def calc_width(sample):
    ...
    width = (korean_count*2.2)+(number_count*1.2)+(english_count*1.2)+(space_count*1)+(special_count*1.2)
    return width
