...
class ExtractInfo():
    def __init__(self):
        ...
        self.EXCEL_FILE_PATH = os.path.join(self.DATA_PATH,"항만정보.xlsx")
        self.PORT_DF = pd.read_excel(self.EXCEL_FILE_PATH,sheet_name="(공통)항만정보")
        self.FACIL_DF = pd.read_excel(self.EXCEL_FILE_PATH,sheet_name="시설코드(부산항만)")
        self.COUNTRY_DF = pd.read_excel(self.EXCEL_FILE_PATH,sheet_name="(공통)국가코드")
    ...
    def set_COUNTRY_info(self,COUNTRIES:str):
        ...
        for country in COUNTRIES:
            if country in ["대한민국","북한(조선민주주의인민공화국)"]:
                continue
            code = self.COUNTRY_DF[self.COUNTRY_DF['한글명'] == country]['2자리코드'].values[0]
