...
def prompt_relevant_query_with_database_prompt(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.relevant_query_parser.get_format_instructions()#.encode('utf-8').decode('unicode_escape')
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
            content=Text2SQLConfig.relevant_query_with_database_prompt.format_map(STATE)
        ),
    ]
...
def prompt_relevant_query_with_search_prompt(STATE):
    ...
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
            content=Text2SQLConfig.relevant_query_with_search_prompt.format_map(STATE)
        ),
    ]
...
def prompt_classify_request(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.check_query_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
...
def prompt_check_port(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.check_port_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
...
def prompt_extract_port(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.extract_port_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
...
def prompt_check_facil(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.check_facil_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
...
def prompt_extract_facil(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.extract_facil_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
    STATE_COPY = copy.deepcopy(STATE)
    STATE_COPY["MESSAGE"] = re.sub(r'_+', ' ', STATE_COPY["MESSAGE"])
    STATE_COPY["MESSAGE"] = re.sub(r'\b\w{1,3}별\b', '', STATE_COPY["MESSAGE"])
    prompt = [
...
def prompt_check_iotype(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.check_io_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
...
def prompt_extract_iotype(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.extract_io_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
...
def prompt_extract_country(STATE):
    STATE['FORMAT_INSTRUCTIONS'] = PARSER_OBJ.extract_country_parser.get_format_instructions().encode('utf-8').decode('unicode_escape')
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
...
def prompt_generate_general_answer(STATE):
    prompt = STATE['HISTORY']+[HumanMessage(content=Text2SQLConfig.generate_general_answer.format_map(STATE['CURRENT']))]
...
def prompt_generate_answer_with_context(STATE):
    ...
    prompt = STATE['HISTORY']+[HumanMessage(content=Text2SQLConfig.generate_answer_with_context.format_map(STATE['CURRENT']))]
...
def prompt_generate_analysis(STATE):
    prompt = STATE['HISTORY']+[HumanMessage(content=Text2SQLConfig.generate_analysis.format_map(STATE['CURRENT']))]
...
def prompt_generate_search_text(STATE):
    ...
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
            content=Text2SQLConfig.generate_search_text.format_map(STATE['CURRENT'])
        ),
    ]
...
def prompt_select_good_search_with_body(STATE):
    prompt = [
        SystemMessage(content=SYSTEM_PROMPT),
        HumanMessage(
            content=Text2SQLConfig.select_good_search_with_body.format_map(STATE)
        ),
    ]
